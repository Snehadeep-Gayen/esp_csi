...
int assignments[NUM_OBS*NUM_VALUES_IN_RAW_CSI];
char mask[2];
...
void* compressor(void*){
    ...
    while(1){
        ...
        // DO KMEANS HERE
        /* Initialisation */
        assert(2 * NO_COMP < NUM_OBS*NUM_VALUES_IN_RAW_CSI); // for faster initialisation this is necessary
        int cluster_indices[NO_COMP];
        srand(1978);
        int index = 0;
        while(index < NO_COMP){
            cluster_indices[index] = (rand() % NUM_OBS*NUM_VALUES_IN_RAW_CSI);
        ...
        for(int i=0; i<NO_COMP; i++)
            cluster_centers[i] = arr2[cluster_indices[i]];
        ...
        while(1){
            ...
            for(int i=0; i<NUM_OBS*NUM_VALUES_IN_RAW_CSI; i++){
                int closest = 0;
                int closest_dist = abs(arr2[i] - cluster_centers[0]);
                for(int j=1; j<NO_COMP; j++){
                    int dist = abs(arr2[i] - cluster_centers[j]);
            ...
            for(int i=0; i<NUM_OBS*NUM_VALUES_IN_RAW_CSI; i++){
                cluster_sums[assignments[i]] += arr2[i];
        ...
        for(int i=0; i<NUM_OBS*NUM_VALUES_IN_RAW_CSI; i++)
            temp_data[i/2] = mask[i%2] & cluster_centers[assignments[i]];
